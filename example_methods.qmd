...
Do infant mortality as a poisson regression (multiply by 10 and convert
to integer).
...
<summary>Table of estimates</summary>

```{r}
bind_rows(sii_results_le, sii_results_im) |> 
  left_join(bind_rows(rii_results_le, rii_results_im),
            by = join_by(year, metric, subgroup, labels)) |>
  mutate(
    Outcome = label_vars[metric],
    Subgroup = subgroup,
    Year = year,
    SII_est = round(sii, 2),
    `SII_95%CI` = glue::glue(
      "{lower}, {upper}",
      lower = round(sii_cl, 2),
      upper = round(sii_cu, 2)
    ),
    RII_est = round(rii, 2),
    `RII_95%CI` = glue::glue(
      "{lower}, {upper}",
      lower = round(rii_cl, 2),
      upper = round(rii_cu, 2)
    ),
    .keep = "none"
  ) |>
  group_by(Outcome) |>
  gt() |>
  tab_spanner_delim("_") |> 
  tab_footnote("*per 1,000 live births")
```
